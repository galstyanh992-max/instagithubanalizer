 ...
 *
 * Browser automation for OpenAI ChatGPT via chatgpt.com.
...
export class ChatGPTAdapter extends BaseBrowserProviderAdapter {
  readonly id = 'chatgpt';
  ...
  async execute(input: BrowserTaskInput, task: BrowserTask): Promise<BrowserTaskOutput> {
    ...
    logs.push(this.log('info', `Starting ChatGPT interaction: ${input.prompt.slice(0, 100)}`, 0));
    ...
    if (!inputSelector) {
      return {
        status: 'failed',
        provider: this.id,
        error: 'Could not find input element on ChatGPT page',
        screenshots,
        logs,
        finalUrl: this.sessionManager.getCurrentUrl(this.config!.id),
        createdAt: task.output.createdAt,
    ...
    if (!fillSuccess) {
      return {
        status: 'failed',
        provider: this.id,
        error: 'Could not fill prompt into ChatGPT input',
        screenshots,
        logs,
        finalUrl: this.sessionManager.getCurrentUrl(this.config!.id),
        createdAt: task.output.createdAt,
