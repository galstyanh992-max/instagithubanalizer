 ...
 *
 * Browser automation for Anthropic Claude via claude.ai.
...
export class ClaudeAdapter extends BaseBrowserProviderAdapter {
  readonly id = 'claude';
  ...
  async execute(input: BrowserTaskInput, task: BrowserTask): Promise<BrowserTaskOutput> {
    ...
    logs.push(this.log('info', `Starting Claude interaction: ${input.prompt.slice(0, 100)}`, 0));
    ...
    const targetUrl = input.url ?? this.config!.url ?? 'https://claude.ai';
    ...
    if (!inputSelector) {
      return {
        status: 'failed',
        provider: this.id,
        error: 'Could not find input element on Claude page',
        screenshots,
        logs,
        finalUrl: this.sessionManager.getCurrentUrl(this.config!.id),
        createdAt: task.output.createdAt,
    ...
    if (!fillSuccess) {
      return {
        status: 'failed',
        provider: this.id,
        error: 'Could not fill prompt into Claude input',
        screenshots,
        logs,
        finalUrl: this.sessionManager.getCurrentUrl(this.config!.id),
        createdAt: task.output.createdAt,
