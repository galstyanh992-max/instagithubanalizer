...
const schema = z.object({ repositoryId: z.string().min(1) });

export const POST = safe(async (req: Request, ctx) => {
  const { id } = await parseParams(ctx);
  const body = await parseJson(req);
  const parsed = schema.safeParse(body);
  if (!parsed.success) return err("repositoryId required", 400);
  try {
    const plan = await integrationPatchPlanner.generate(parsed.data.repositoryId, id);
    return ok({ plan });
  } catch (e) {
