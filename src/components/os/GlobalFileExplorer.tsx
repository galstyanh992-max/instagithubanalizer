...
/**
 * Global File Explorer overlay — slides up from the bottom of the viewport.
 * Toggled by the Files button in the MacDock (via Zustand `filesOpen`).
 * Rendered once in the root layout so it is available on every route.
 */
export function GlobalFileExplorer() {
  const open = useUiStore((s) => s.filesOpen);
  const setOpen = useUiStore((s) => s.setFilesOpen);
            ...
            <div className="max-w-6xl mx-auto px-4 pt-2">
              <div className="flex items-center justify-between px-3 py-2 rounded-t-xl bg-zinc-950/90 border border-b-0 border-cyan-400/30 backdrop-blur-xl">
                <div className="flex items-center gap-2 text-cyan-300 text-[11px] font-mono uppercase tracking-wider">
                  <FolderOpen className="h-3.5 w-3.5 drop-shadow-[0_0_5px_currentColor]" />
                  Files
                </div>
                <button
                  type="button"
