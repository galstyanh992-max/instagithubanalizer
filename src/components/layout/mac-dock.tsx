  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 group">
      ...
      <motion.div 
        ...
        className="flex items-end gap-2 p-2 rounded-2xl bg-zinc-900/60 border border-zinc-700/50 backdrop-blur-xl shadow-2xl transition-all hover:bg-zinc-900/90"
      >
        {items.map((item, i) => {
          ...
          return (
            <div key={i} className="relative group/item flex flex-col items-center">
              {/* Tooltip */}
              <div className="absolute -top-10 bg-zinc-800 text-zinc-100 text-[10px] px-2 py-1 rounded opacity-0 group-hover/item:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
                {item.label}
              </div>
              
              <button className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl bg-zinc-800/80 border border-zinc-700 hover:bg-zinc-700 flex items-center justify-center hover:-translate-y-2 transition-all duration-200 hover:shadow-[0_10px_20px_rgba(0,0,0,0.5)]">
