...
// --- NODES DATA ---
const NODES_DATA = [
  { id: "projects", label: "Projects", icon: FolderKanban, status: "12 Active", color: "#22d3ee", pos: [1.3, 0.6, 0.9], hasRings: true },
  { id: "agents", label: "Agents", icon: Users, status: "16 Running", color: "#a3e635", pos: [1.1, 1.3, -0.6], hasRings: false },
  { id: "memory", label: "Memory", icon: HardDrive, status: "1.2M Docs", color: "#a855f7", pos: [-1.3, 0.8, 0.6], hasRings: true },
  { id: "github", label: "GitHub", icon: Github, status: "Connected", color: "#a1a1aa", pos: [-1.1, -1.1, 0.8], hasRings: false },
  { id: "database", label: "Database", icon: Database, status: "PostgreSQL", color: "#3b82f6", pos: [0.8, -1.3, 0.6], hasRings: true },
  { id: "browser", label: "Browser", icon: Globe, status: "3 Sessions", color: "#fbbf24", pos: [-1.3, -0.6, -0.8], hasRings: false },
  { id: "terminal", label: "Terminal", icon: Terminal, status: "2 Active", color: "#a3e635", pos: [1.3, -0.6, -0.8], hasRings: false },
  { id: "tools", label: "Tools", icon: Wrench, status: "24 Available", color: "#22d3ee", pos: [0, 1.6, 0.6], hasRings: true },
];
`;

code = code.replace(/\/\/ --- NODES DATA ---[\s\S]*?\];/, newNodesData.trim());

// Replace InteractiveNode
...
function InteractiveNode({ data, selected, onClick }: any) {
  ...
  useFrame((state) => {
    ...
    // Orbital gentle floating
    ref.current.position.y = data.pos[1] + Math.sin(t * 1.5 + data.pos[0]) * 0.05;
    ref.current.position.x = data.pos[0] + Math.cos(t * 1.5 + data.pos[1]) * 0.05;
